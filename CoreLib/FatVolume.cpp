...
EFatCode CFatVolume::QueryNextEntry(SFatQueryState* psQuery, bool bBufferLocked, bool bFirstEntry)
{
	...
	do
	{
		...
		if (psQuery->psCurrentEntryRaw->uEntry.sFatRawCommon.uiAttributes == FAT_ATTR_LONG_NAME && !IS_FREE_DIRECTORY_ENTRY(psQuery->psCurrentEntryRaw))
		{
			...
			// if this is the LFN that we're expecting then process it, otherwise we'll have to wait for another 1st LFN entry otherwise read the LFN
...
uint32 CFatVolume::GetEntryCluster(SFatDirectoryEntry* psEntry)
{
	...
	uiEntry = psEntry->raw.uEntry.sFatRawCommon.uiFirstClusterLowWord | psEntry->raw.uEntry.sFatRawCommon.uiFirstClusterHighWord << 16;
...
EFatCode CFatVolume::FindFirstFATEntry(char* szParentPath, uint8 uiAttributes, SFatDirectoryEntry** ppsDirectoryEntry, SFatFileSystemQuery* psQuery)
{
	...
	if (*psQuery->sCurrentEntry.name != 0)
	{
		if (*psQuery->sQuery.auiLongFilename != 0)
		{
			...
			{
				psQuery->sCurrentEntry.name[uiIndex] = (uint8)psQuery->sQuery.auiLongFilename[uiIndex];
...
EFatCode CFatVolume::CreateFATEntry(SFatRawDirectoryEntry* psParentDirectory, char* szName, uint8 uiAttributes, uint32 uiEntryCluster, SFatDirectoryEntry* psNewEntry)
{
	...
	// there's no fat entry that points to the 1st cluster of a directory's cluster chain but we'll create a fake fat entry from the 1st cluster data on the
	// directory entry so that we can handle the 1st cluster with the same code as all other clusters in the chain.
	...
	// if no parent was specified then we create the fake fat entry from the root directory's cluster address found on the volume structure
	else
	{
