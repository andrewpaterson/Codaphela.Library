...
TRISTATE GetCPPIdentifierCharacter(CTextParser* pcTextParser, char* pc, bool bFirst);
...
TRISTATE ParseCPPExactIdentifier(CTextParser* pcTextParser, char* szIdentifier)
{
	...
	for (;;)
	{
		if (szIdentifier[iPos] == 0)
		{
			...
			if (!pcTextParser->IsOutside())
			{
				tResult = GetCPPIdentifierCharacter(pcTextParser, &cCurrent, iPos == 0);
				if (tResult == TRITRUE)
				{
...
TRISTATE ParseCPPIdentifier(CTextParser* pcTextParser, char* szIdentifier, size* piLength)
{
	...
	for (;;)
	{
		if (!pcTextParser->IsOutside())
		{
			tResult = GetCPPIdentifierCharacter(pcTextParser, &c, bFirst);
			if (tResult != TRITRUE)
			{
				if (bFirst)
				{
